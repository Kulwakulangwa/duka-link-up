...
const MAX_IMAGE_BYTES = 10 * 1024 * 1024;import { createFileRoute, Link, useNavigate, useParams } from "@tanstack/react-router";
import { useEffect, useState } from "react";
import { supabase } from "@/integrations/supabase/client";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
...
export function ProductForm({ mode, productId }: { mode: "add" | "edit"; productId?: string }) {
  ...
  const [saving, setSaving] = useState(false);
  const [loading, setLoading] = useState(true);
  const [maxProducts, setMaxProducts] = useState<number>(FREE_PRODUCT_LIMIT);
    ...
    async function load() {
      try {
        ...
        if (mode === "edit") {
          ...
          if (count && count >= limit) {
            toast.error(`Free plan limited to ${limit} products`);
            navigate({ to: "/dashboard" });
            return;
          }
        }
